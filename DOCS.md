...
Its titlebar shows the real mark whenever `util/Asset.lua` has cached the logo on
disk from an earlier session (`getcustomasset`, no network involved), and the
accent square + brand initial when it hasn't — a first-ever run, or an executor
with no file access. Uploading the mark as a Roblox asset and adding its
`rbxassetid://` to `Theme.Brand.logo` would make it unconditional here.
